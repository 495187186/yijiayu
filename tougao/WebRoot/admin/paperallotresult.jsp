...
function checkDate() 
{
  ...
  <body>
    ...
    %>
    <form action="admin/paperallotresult.jsp" method="post" name="searchpaper">
            ...
            <td>
               <table width="100%"  border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#cccccc">
                  <tr class="kbj">
                      ...
                      <td width="4%" height="30" bgcolor="#d5f1f2" ><div align="center" >专家</div></td>
                      <td width="8%" height="30" bgcolor="#d5f1f2" >
                          <div align="center">
    ...
    <form action="admin/paperallotresult.jsp"" method="post" name="myform">
        <table width="98%" border="2" bordercolor="#3399cc" align="center" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF" >
          <tr>
            <td height="30">
               <div align="center">
                  <FONT color=#000000><B>总计：</B></FONT>&nbsp;&nbsp;已收到论文：<FONT color=#ff0000><STRONG><%=paperList.size() %></STRONG></FONT> 篇，已指派：<FONT color=#ff0000><STRONG><%=paperallot %></STRONG></FONT> 篇，尚未指派：<FONT color=#ff0000><STRONG><%=paperList.size()-paperallot %></STRONG></FONT> 篇，预付审稿费：<FONT color=#ff0000><STRONG><%=paperreward %></STRONG></FONT> 元。
               </div>
            </td>
         </tr>
			...
			<td width="100%" align="center">
			   <table width="100%" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC">
				  <tr bgcolor="#d5f1f2" class="bjl">
					 ...
					 <td width="23%" height="30"><div align="center">论文题目</div></td>
				   ...
				   <%
			         ...
			         if((papername==null||papername.equals("")||papername.equals("null"))&&(begindate==null||begindate.equals("")||begindate.equals("null"))&&(enddate==null||enddate.equals("")||enddate.equals("null"))&&(uploader==null||uploader.equals("")||uploader.equals("null"))&&(writer==null||writer.equals("")||writer.equals("null"))&&(expert==null||expert.equals("")||expert.equals("null"))){
			             flag=1;
			      ...
			      <%
				      for(int i=start;i<((start+pageSize)>allRow?allRow:(start+pageSize));i++)
                         {
                            AllotPaper obj = (AllotPaper)list.get(i);                           
                            if(i%2==0){
				  ...
				  %> 
					    ...
					    <td width="23%" height="30"><div align="center"><%=obj.getPaperName() %></div></td>
				  ...
				  <%
			             ...
			             list=allotpaperDao.findByPaperUploader(uploader);
			             if(list.size()==0){
					       flag = 0;
					       out.println("<script language=javascript>");
                           out.print("alert('查询结果为空！');");
				  ...
				  %> 
				   ...
				   <%
			             ...
			             list=allotpaperDao.findByPaperWriter(writer);
			             if(list.size()==0){
					       flag = 0;
					       out.println("<script language=javascript>");
                           out.print("alert('查询结果为空！');");
				  ...
				  <%
			             ...
			             list=allotpaperDao.findByExpertName(expert);
			             if(list.size()==0){
					       flag = 0;
					       out.println("<script language=javascript>");
                           out.print("alert('查询结果为空！');");
				  ...
				  <%
			             ...
			             list=allotpaperDao.findByAllotTime(begindate,enddate);
			             if(list.size()==0){
					       flag = 0;
					       out.println("<script language=javascript>");
                           out.print("alert('查询结果为空！');");
       ...
       %>
           ...
           <a href="admin/paperallotresult.jsp?page=<%="1" %>&&papername=<%=papername %>&&begindate=<%=begindate %>&&enddate=<%=enddate %>&&uploader=<%=uploader %>&&writer=<%=writer %>&&expert=<%=expert %>">首页 </a>&nbsp;&nbsp;
        <%
           if(currentPage>1){
        %>
           <a href="admin/paperallotresult.jsp?page=<%=currentPage-1%>&&papername=<%=papername %>&&begindate=<%=begindate %>&&enddate=<%=enddate %>&&uploader=<%=uploader %>&&writer=<%=writer %>&&expert=<%=expert %>">上一页</a>
        <%
           }else{
        %>
                             上一页
        ...
        %>
           <a href="admin/paperallotresult.jsp?page=<%=currentPage+1 %>&&papername=<%=papername %>&&begindate=<%=begindate %>&&enddate=<%=enddate %>&&uploader=<%=uploader %>&&writer=<%=writer %>&&expert=<%=expert %>">下一页 </a>
        <%
           }else{
        %>
                             下一页 
        ...
        %>
          &nbsp;&nbsp;
          <a href="admin/paperallotresult.jsp?page=<%=totalPage %>&&papername=<%=papername %>&&begindate=<%=begindate %>&&enddate=<%=enddate %>&&uploader=<%=uploader %>&&writer=<%=writer %>&&expert=<%=expert %>">尾页 </a>&nbsp;&nbsp;
