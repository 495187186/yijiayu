...
function checkForm() 
{
  ...
  <body>
    <%
       ...
       String strPage;
       List<ReviewPaper> list = null; 
       list = reviewpaperDao.findReviewPaper(no,"0");
	   allRow = list.size();
    ...
    <form action="expert/managereviewpaper.jsp"" method="post" name="myform">
		  ...
		  <tr>
			 <td height="25">
			    <table width="100%" height="25" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#3399cc">
				   <tr>
                      <td background="images/td_bg.gif" height=25 align="center"><FONT color=#000000><STRONG>全部待审论文列表</STRONG></FONT></td>
                  </tr>
			   </table>	
			 </td>
		 </tr>	
			...
			<td width="100%" align="center">
			   <table width="100%" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC">
				  <tr bgcolor="#d5f1f2" class="bjl">
					 ...
					 <td width="25%" height="30"><div align="center">论文题目</div></td>
				  ...
				  <%
				     for(int i=start;i<((start+pageSize)>allRow?allRow:(start+pageSize));i++)
                         {
                            ReviewPaper obj = (ReviewPaper)list.get(i);                           
                            if(i%2==0){
				  ...
				  %> 
					 ...
					 <td width="25%" height="30"><div align="center"><a href="expert/reviewpaper.jsp?expertno=<%=no %>&&papername=<%=URLEncoder.encode(obj.getPaperName(), "UTF-8") %>&&papertype=<%=obj.getPaperType() %>&&uploaddate=<%=obj.getUploadDate().substring(0,obj.getUploadDate().length()-2) %>&&allotdate=<%=obj.getAllotDate().substring(0,obj.getAllotDate().length()-2) %>&&paperreward=<%=obj.getPaperReward() %>"><%=obj.getPaperName() %></a></div></td>
				     ...
				     <td width="15%" height="30"><div align="center"><%=obj.getPaperDeadline().substring(0,obj.getPaperDeadline().length()-2) %></div></td>
				     <td width="10%" height="30"><div align="center"><%=obj.getPaperReward() %></div></td>		
				     <td width="5%" height="30"><div align="center"><%=obj.getPaperFlag().equals("0")?"无":obj.getIsAgreed().equals("1")?"同意":"拒绝" %></div></td>
				     <td width="16%" height="30">
				        <div align="center">
				           <%
						      if(obj.getPaperFlag().equals("0")){//还未同意或拒绝
						   %>
						         <img height="15" width="15" src="images/smile.gif"> 
				                 <a href="expert/agreeornot.jsp?result=<%="1" %>&&Id=<%=obj.getReviewPaperId() %>" onclick="if(confirm('您确定同意审批吗?')==false)return false;">同意</a> 
				                 &nbsp; &nbsp;
				                 <img height="15" width="15" src="images/sad.gif">
						         <a href="expert/agreeornot.jsp?result=<%="0" %>&&Id=<%=obj.getReviewPaperId() %>" onclick="if(confirm('您确定拒绝审批吗?')==false)return false;">拒绝</a>  
						   <%
						         }else{
       ...
       %>
           ...
           <a href="expert/managereviewpaper.jsp?page=<%="1" %>">首页 </a>&nbsp;&nbsp;
        <%
           if(currentPage>1){
        %>
           <a href="expert/managereviewpaper.jsp?page=<%=currentPage-1%>">上一页</a>
        <%
           }else{
        %>
                             上一页
        ...
        %>
           <a href="expert/managereviewpaper.jsp?page=<%=currentPage+1 %>">下一页 </a>
        <%
           }else{
        %>
                             下一页 
        ...
        %>
          &nbsp;&nbsp;
          <a href="expert/managereviewpaper.jsp?page=<%=totalPage %>">尾页 </a>&nbsp;&nbsp;
