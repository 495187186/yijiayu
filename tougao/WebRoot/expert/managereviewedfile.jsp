...
function checkForm() 
{
  ...
  <body>
    <%
       ...
       String strPage;
       List<ReviewedFile> list = null; 
       list = reviewedfileDao.findReviewedFile(no);
	   allRow = list.size();
    ...
    <form action="expert/managereviewpaper.jsp"" method="post" name="myform">
		  ...
		  <tr>
			 <td height="25">
			    <table width="100%" height="25" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#3399cc">
				   <tr>
                      <td background="images/td_bg.gif" height=25 align="center"><FONT color=#000000><STRONG>全部已审已审列表</STRONG></FONT></td>
                  </tr>
			   </table>	
			 </td>
		 </tr>	
			...
			<td width="100%" align="center">
			   <table width="100%" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC">
				  <tr bgcolor="#d5f1f2" class="bjl">
					 ...
					 <td width="10%" height="30"><div align="center">材料类别</div></td>
					 <td width="15%" height="30"><div align="center">材料指派日期</div></td>
				     <td width="15%" height="30"><div align="center">材料审批日期</div></td>
				  ...
				  <%
				     for(int i=start;i<((start+pageSize)>allRow?allRow:(start+pageSize));i++)
                         {
                            ReviewedFile obj = (ReviewedFile)list.get(i);                           
                            if(i%2==0){
				  ...
				  %> 
					 ...
					 <td width="30%" height="30"><div align="center"><a href="expert/reviewedfile.jsp?expertno=<%=no %>&&docname=<%=URLEncoder.encode(obj.getDocName(), "UTF-8") %>&&doctype=<%=obj.getDocType() %>&&allotdate=<%=obj.getAllotTime().substring(0,obj.getAllotTime().length()-2) %>&&reviewtime=<%=obj.getReviewTime().substring(0,obj.getReviewTime().length()-2)%>"><%=obj.getDocName() %></a></div></td>
					 <td width="10%" height="30"><div align="center"><%=obj.getDocType().equals("0")?"学校建设":"教学项目" %></div></td>
       ...
       %>
           ...
           <a href="expert/managereviewedfile.jsp?page=<%="1" %>">首页 </a>&nbsp;&nbsp;
        <%
           if(currentPage>1){
        %>
           <a href="expert/managereviewedfile.jsp?page=<%=currentPage-1%>">上一页</a>
        <%
           }else{
        %>
                             上一页
        ...
        %>
           <a href="expert/managereviewedfile.jsp?page=<%=currentPage+1 %>">下一页 </a>
        <%
           }else{
        %>
                             下一页 
        ...
        %>
          &nbsp;&nbsp;
          <a href="expert/managereviewedfile.jsp?page=<%=totalPage %>">尾页 </a>&nbsp;&nbsp;
